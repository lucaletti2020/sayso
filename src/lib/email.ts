...
// domain (e.g. "Chatterbox <hello@yourdomain.com>") once it's set up.
const FROM = process.env.EMAIL_FROM ?? "Chatterbox <onboarding@resend.dev>";
...
function layout(heading: string, bodyHtml: string, cta?: { label: string; url: string }) {
  ...
  return `<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#0b1628;">
    <div style="font-size:22px;font-weight:700;margin-bottom:24px;">🔊 Chatterbox</div>
    ...
    ${button}
    <p style="font-size:12px;color:#9aa7b4;margin-top:32px;">Chatterbox — English conversation practice for work.</p>
  </div>`;
}
...
export async function sendReengagementEmail(to: string, name?: string | null) {
  ...
  await send(
    ...
    layout(
      ...
      `<p>You signed up for Chatterbox but haven't tried a conversation yet. It only takes a few minutes — speak with your AI practice partner about a real work situation and get instant feedback.</p>
